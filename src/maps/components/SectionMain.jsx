...
const initWindowViews = () => {
    ...
    if (!div1 || !div2 || !floatWindowPrimary) return;

    // 1. APLICAR ESTADOS DE INMEDIATO (sin transición)
    // Desactivamos animaciones temporalmente para ocultar div1 al instante
    ...
    // Ajustar la altura sin animación inicial
    floatWindowPrimary.style.height = div2.scrollHeight + "px";

    // Reestablecer animaciones tras el render inicial
    requestAnimationFrame(() => {
        div1.style.transition = "";
        div2.style.transition = "";
    });

    // 2. ACCIÓN: Mostrar div1 (Volver)
    const handleClose = () => {
        div2.classList.remove("visible");
        setTimeout(() => {
            div1.classList.remove("hidden");
            floatWindowPrimary.style.height = div1.scrollHeight + "px";
        }, 250);
    };

    // 3. ACCIÓN: Mostrar div2 (Ir a ventana 2)
    const handleToggle = () => {
        div1.classList.add("hidden");
        setTimeout(() => {
            div2.classList.add("visible");
    ...
    return () => {
        ...
        if (btnToggle) btnToggle.removeEventListener("click", handleToggle);
    };
};
const SectionMain = ({
    translator,
